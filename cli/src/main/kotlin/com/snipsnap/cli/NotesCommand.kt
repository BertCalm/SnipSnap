...
object NotesCommand {
    ...
    fun run(args: List<String>, out: PrintStream): Int {
        ...
        // Under a labeled crate, the notes lead with the catalog number.
        val catalog = com.snipsnap.shell.Label.forKit(kitDir)
        val text = LinerNotes.render(kit, kitDir, catalog)
        out.print(text)

        val destDir = opts["--out"]?.let { File(it).apply { mkdirs() } } ?: kitDir
        val file = LinerNotes.writeTo(kit, kitDir, File(destDir, LinerNotes.FILE_NAME), catalog)
