...
/**
 * `snipsnap roll <kit-dir> <pad>` — the hit's own head struck again on the
 * grid, at a note division (`--rate 1/16`) and the kit's tempo (`--bpm`
 * sets it first). `--amount` is how much of the hit the roll replaces,
 * `--undo` pops the earlier audio back out of the bin.
 */
object RollCommand {
    ...
    fun run(args: List<String>, out: PrintStream): Int {
        ...
        val dirArg = opts.positional.getOrNull(0)
            ?: throw CliError("roll wants a kit and a pad: snipsnap roll <kit-dir> A02 [--rate 1/16] [--bpm 92] [--amount 0..1]")
        ...
        if (opts.positional.size > 2) throw CliError("roll takes a kit and a pad, nothing more")
        ...
        val done = model.keyedPad(slot, "rolled", amount, dials = Keyed.Dials(division = division))
        model.save()
        val period = Wobble.periodSec(model.keyedContext().bpm, division)
        out.println(
            "pad $padArg rolled at %s (%.0f ms a repeat)%s - original in the bin, recipe recorded (undo: --undo)".format(
                Locale.ROOT,
