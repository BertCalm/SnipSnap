...
**Transport** one row of h48 buttons (field fill, r6, gray-edge; accent border+ink when lit): `↺ TOP` · `BPM −` · `BPM +` · `+ PAD` · `+ SNIP`. `+ SNIP` swaps the panel for the SNIPS shelf as a list (`WHICH SNIP GOES ROUND A RING?`, h40 LCD rows in lcd-alt, CLOSE chip) and is disabled when the shelf is empty — a picker, never an arrow-cycle. Footer Silkscreen 8px ink3, two lines: `TAP A RING TO PICK IT · HOLD TO SOLO · TAP A CELL FOR A HIT, HOLD IT FOR AN ACCENT · SHORTEST RING INSIDE COMES ROUND FIRST.`
