...
| CHOP | ChopShop.dc.html | chip tap = cycle class label, "YOU ✓"; under RE-CHOP / SEND TO GRID, `ONTO <kit> · BANK X` (SecondaryButton, full width) when the open kit has an empty bank — lands the same arrangement on that bank through `KitBuilderModel.landArranged` (first sixteen; toast `Copy.landedOnto` counts what didn't fit) and opens KIT on the bank it landed on (`KitScreen.bankRequest`, a one-shot ask). **CHOP CONTROLS round 1** (`docs/CHOP_CONTROLS.md`): a tape strip under the header with a marker at every cut, redrawn live; a `CUT` GroupBox under it (closed to `12 HITS · BY HITS · FINE`): BY HITS / GRID, ◀ count ▶ + AUTO (the knee), EAR COARSE / NORMAL / FINE, CUT EARLY / ON / LATE — every change a fresh chop with corrected chips carried across (`rechopKeeping`); MERGE WITH n+1 and SPLIT AT ITS NEXT HIT at the top of a chip's picker panel; the header reads `15 SLICES — BY HITS · CUT EARLY · EDITED`. **Round 2** (§8): an ON THE GRID row on the bench (OFF / 8TH / 16TH / 32ND under `ON THE GRID · CUTS ON THE PULSE · 94 BPM`, or NO TEMPO HEARD) snapping every cut to the source's own pulse, anchored on the first cut, never after an attack; a FOLD segment between CLASSIC and MELODIC — one pad per sound, the repeats cycling under it as a chain pad (`ArrangedPad.takes`), rows tagged `×4 TAKES` / `TAKE 2 OF 4 · = 3`, strip `FOLD: 16 SLICES → 5 PADS.`, SEND `16 SLICES FOLDED ONTO 5 PADS.` |
...
| BANK B | TapeOS Oilslick.dc.html (KIT header + action row) | W4.3 — EVIL TWINS button fills bank B (twins of assigned bank-A pads, treatment tag top-right in accent 7px Silkscreen, colour-washed fill + 2px dashed class border); button becomes REROLL after first run (reshuffles treatments); a BANK A · n / BANK B · n row under the KIT header flips the grid — always drawn, B reading `BANK B · EMPTY` (ink3) until something lands there; an empty B flips like a full one (its blanks take a hold-to-capture, a SNIPS → PAD landing or a chop ONTO it, the same as A's) and toasts `Copy.bankEmpty(letter)` naming those ways as the blanks come up; the letter in the header is a readout. Twins are stamped `twinOf` (`KitBuilderModel.TWIN_OF`); REMIX BANK B refuses over the user's own bank-B pads (`Copy.TWINS_KEEP_OWN`, `KitBuilderModel.ownPadsOnBankB`) rather than wiping them; PAD SHEET opens on a pad of either bank (September UAT, finding 11) |
| DUST | PAD SHEET treatment card (sixth chip) + `DUST FROM ▸` row under it + KIT action row `DUST ALL ▸` | `docs/DUST.md` — the tape's own floor (HISS, the quietest window as a seamless loop), room (ROOM, the averaged noise part of every hit's tail, 8 ms pre-delay) and crackle (CRACKLE, the Capture Doctor's clicks off the ghosts as 4 ms grains, sprinkled by a generator seeded from the hit) under a pad via `KitBuilderModel.dustPad`, bin-backed, recipe `{verb:dust, amount, tape}`; one AMT, three curves (`Dust.Curves`: room linear, hiss dB-linear, crackle the square); the print is cached beside the tape in `snips/.dust/` (three files); `DUST FROM ▸` opens the shelf's tapes inline on the bench and dusts the pad from the picked one, the recipe keeping it; DUST ALL dusts every pad at 50 from its own tape else the kit's; refusals in the app's words (no tape, tape gone, nothing between the hits, no tapes on the shelf) |
| BREED | KIT action row + MY KITS pick header | XX2 — `BREED ▸ MIX TWO KITS` (`Copy.BREED_BUTTON`, constant: BREED always opens the picker) with `Copy.breedSubtitle` (count from `Breed.recipePads`, n of m pads) under it in pixelSmall ink3; tap → MY KITS with header `PICK A KIT TO CROSS WITH <name>`; tapping a kit runs `Breed.breed` into a new child beside A unless `Breed.crossable` is empty, which toasts `Copy.BREED_NOTHING_TO_CROSS` and keeps the pick armed; HELP carries one line each for BREED and REMIX BANK B |
...
Layout top→bottom: header row (◄ KIT 64px · LCD `PAD A02` + class chip in pad colour · ◀ BEFORE 92px, only while a treatment is on and the take before it is still in the bin — plays that take through the same SHAPE, an A/B against · ▶ HIT 64px, 2px pad-colour border + glow); waveform LCD h64 (44 bars, exp-decay envelope in pad colour — loops decay slower; W12 draws these from `PeaksPyramid` for real); provenance line (Y1.3 `source`: `twin of A03` / `bred from Mother × Father` first when a door made the pad from others, then file @ time · length · bin status); three stepper-sliders LEVEL −24..+6 dB / PAN L50..R50 / TUNE ±12 st (fill bar in pad colour, VT323 readout); toggle row ONE-SHOT · CHOKE GRP 1 · GHOSTS (W5.3 — engaged = pad-colour fill; hats default choke ON per KIT_BEST_PRACTICES); TREATMENT card (Y4.3): NONE/CRUSH/TAPE/DIRT/SMEAR/DUST segmented (DUST = the tape's own hiss, room and crackle under the pad, from the pad's tape else the kit's — `docs/DUST.md`; provenance line reads `dusted from <tape>`; under the card a `DUST FROM ▸` row reads which tape and opens the shelf's tapes inline to borrow another's) + AMT 0-100 in 5s, opening at 70 on an untreated pad (`PadSheet.DEFAULT_AMOUNT` — 35 was too quiet to tell on the phone); picking a treatment auditions the treated pad the moment it lands (`auditionOnRefresh`, played after the model swap so the voice belongs to the fresh model); action row RE-TRIM ▸ (→ TAPE opened on the pad's own snip with IN/OUT on its cut, header `RE-TRIM A02 · <file>`, primary button BACK ONTO A02 — `docs/RETRIM.md`; a pad with no tape toasts why) · MAKE INSTRUMENT (W2.3 — enabled styling only on tonal pads, refusal toast otherwise) · EJECT → BIN (X2, red border `#6A2020`/`#C86050`); prev/next pad nav cycling assigned pads only.

Per-pad state is a prefs map keyed by pad id — level/pan/tune/oneShot/choke/ghosts/treat/amt — defaults: −2 dB, C, 0 st, one-shot ON, choke ON for hats, ghosts OFF, NONE @ 70%.
...
Shipped copy adds: "MELODIC. THE PADS BECOME A SCALE, LOW LEFT." · "Am SET. TONAL PADS RETUNE ON ASSIGN — THE KICK IS UNTOUCHED." · "KEY OFF. EVERYTHING LANDS AS CAPTURED." · "TEACHING ON. THE MACHINE LEARNS FROM YOUR CORRECTIONS." · "TEACHING OFF. THE MACHINE STOPS TAKING NOTES." · "BANK B REMIXED: EVERY PAD'S EVIL TWIN. RECIPES KEPT." · "BANK B REROLLED. SAME SEED, DIFFERENT SINS." · "BANK B: EMPTY. HOLD A PAD TO CAPTURE ONTO IT, OR TO PLACE A SNIP FROM SNIPS. A CHOP CAN LAND HERE. REMIX BANK B ▸ DEALS TWINS HERE TOO." · "BANK B HOLDS YOUR OWN PADS. THEY STAY UNTOUCHED — REMIX ONLY DEALS ONTO AN EMPTY B. CLEAR THEM FIRST, OR KEEP THE PAGE." · "CRUSH ON A02, OVER THE LAST ONE — NO ORIGINAL IN THE BIN TO SWAP FROM. VERSIONS ▸ ROLLS BACK." · "NO TAPE TO TAKE DUST FROM: THIS PAD, AND THIS KIT, NEVER CAME OFF ONE." · "NOTHING BETWEEN THE HITS ON THAT TAPE. NO DUST TO TAKE." · "3 PADS DUSTED FROM THE KIT'S OWN TAPE." · "NO TAPES ON THE SHELF TO TAKE DUST FROM. RECORD OR IMPORT ONE FIRST." · "DUST FROM 'KITCHEN' ON A02. ORIGINAL SLEEPS IN THE BIN." · "NOTHING WOULD CROSS: NO PAD HERE HAS A PATCH OR TREATMENT, OR A PARTNER THERE WITH ONE TO BORROW. TREAT A PAD FIRST." · "T3 RESTORED. THE PAST, REPLAYED." · "BACK FROM THE BIN. NO QUESTIONS ASKED." · "BIN EMPTIED. THE MACHINE FORGETS, AS ASKED." · "HUMANIZED. NOBODY PLAYS LIKE A ROBOT." · "FORKED TO PROG E. A–D STAY UNTOUCHED." · "BAR WIPED. THE MACHINE FORGIVES."
Shipped copy: "TAPE ROLLING. GO STEAL A SOUND (LEGALLY)." · "TAPED. NO TAKEBACKS." · "IT'S OURS NOW." · "CLEAN CUT. NICE EARS." · "SNIP! LAST 60s KEPT." · "EJECTED. TAPE IS KEPT." · "PENCIL REWIND. OLD SCHOOL." · "DUB DONE. SOUNDS 3% WARMER NOW." · "FRESH TAPE. SMELLS LIKE FERRIC OXIDE." · "RE-CHOPPED. THE MACHINE APOLOGIZES FOR SLICE 3." · "ONLY 6 HITS HEARD AT THIS EAR. FINE HEARS MORE." · "AUTO: 8 HITS, WHERE THE REAL ONES END AND THE SCRAPS BEGIN." · "SLICES 3 AND 4 ARE ONE NOW." · "SLICE 3 IS TWO NOW." · "NO SECOND HIT INSIDE SLICE 3. NOTHING TO SPLIT." · "NO TEMPO HEARD ON THIS TAPE. THE GRID NEEDS A PULSE." · "FOLDED. ONE PAD PER SOUND; THE REPEATS CYCLE UNDER IT." · "16 SLICES FOLDED ONTO 5 PADS." · "SNACK BAR. SHIPPED OUT OF RESPECT."
