...
| CHOP | ChopShop.dc.html | chip tap = cycle class label, "YOU ✓"; under RE-CHOP / SEND TO GRID, `ONTO <kit> · BANK X` (SecondaryButton, full width) when the open kit has an empty bank — lands the same arrangement on that bank through `KitBuilderModel.landArranged` (first sixteen; toast `Copy.landedOnto` counts what didn't fit) and returns to KIT — on bank A; the toast and the `BANK B · n` row say where it landed |
