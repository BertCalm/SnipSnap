...
| CHOP | ChopShop.dc.html | chip tap = cycle class label, "YOU ✓"; under RE-CHOP / SEND TO GRID, `ONTO <kit> · BANK X` (SecondaryButton, full width) when the open kit has an empty bank — lands the same arrangement on that bank through `KitBuilderModel.landArranged` (first sixteen; toast `Copy.landedOnto` counts what didn't fit) and opens KIT on the bank it landed on (`KitScreen.bankRequest`, a one-shot ask). **CHOP CONTROLS round 1** (`docs/CHOP_CONTROLS.md`): a tape strip under the header with a marker at every cut, redrawn live; a `CUT` GroupBox under it (closed to `12 HITS · BY HITS · FINE`): BY HITS / GRID, ◀ count ▶ + AUTO (the knee), EAR COARSE / NORMAL / FINE, CUT EARLY / ON / LATE — every change a fresh chop with corrected chips carried across (`rechopKeeping`); MERGE WITH n+1 and SPLIT AT ITS NEXT HIT at the top of a chip's picker panel; the header reads `15 SLICES — BY HITS · CUT EARLY · EDITED` |
