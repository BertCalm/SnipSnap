...
| DUST | PAD SHEET treatment card (sixth chip) + KIT action row `DUST ALL ▸` | `docs/DUST.md` — the tape's own floor (HISS, the quietest window as a seamless loop) and room (ROOM, the averaged noise part of every hit's tail, 8 ms pre-delay) under a pad via `KitBuilderModel.dustPad`, bin-backed, recipe `{verb:dust, amount, tape}`; the print is cached beside the tape in `snips/.dust/`; DUST ALL dusts every pad at 50 from its own tape else the kit's; refusals in the app's words (no tape, tape gone, nothing between the hits) |
