...
Pages three through twelve were added after the build, the other way
...
shelf-level catch-all for every captured WAV) and the boards check it.
...
GRAIN FIELD, SPLIT, SHARE/BACKUP, KEYS, PAD CAPTURE and SNIPS all held
up the same way: page eight closed `DESIGN_GAP.md`'s last "still
undrawn" line, page nine checked EEE9, page ten checked KEYS, page
eleven checked PAD CAPTURE, and page twelve checks SNIPS — screens an
archaeology pass across every `app/.../ui/` file turned up with no
board of their own. `DeletedKitsScreen` remains undrawn past this pass.
