...
- `THE MIC ISN'T LISTENING. LISTEN · MIC ON KITS, THEN HUM.` · `THE INSIDE IS LISTENING, NOT THE MIC. STOP ON KITS, THEN LISTEN · MIC, THEN HUM.` (the INSIDE's session has to be stopped first: KITS shows STOP in the button's place while one runs)
