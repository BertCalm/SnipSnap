...
| F4.5 | ✓ done: FORGET for presets (`docs/WORKSHOP.md`, WS5's follow-up) — the app's one rule for a delete, kept: a hold on a YOURS chip asks first (`Copy.presetForgetAsk`, CANCEL beside FORGET → BIN in the bin's red), the preset moves to a `binned` list in `presets.json` with the same thirty days every bin promises (`UserPresets.BIN_DAYS`, pinned to `Reversal.DAYS` by `ReversalTest`; swept on launch), and DELETED PRESETS ▸ N WAITING under the strips is the door back — one row per forgotten preset with engine, voice, age and days left, RESTORE landing it at the end of YOURS under its own name or a fresh one that fits if the name was saved again meanwhile. No EMPTY THE BIN NOW, on purpose: a preset costs nothing to keep, and every irreversible site is stack evidence. The save slip's note says how a preset leaves | CORE + APP | S | a forgotten preset is off the strip and in the bin; a save in between carries the bin; restore returns it under its name, or `NAME 2` trimmed to fourteen letters when taken; the sweep goes at the boundary the readout counts to; the bin is never a roster line but rides in the bench zip |
| F4.4 | ✓ done: SAVE AS PRESET (`docs/WORKSHOP.md`, WS5 — the one tool on that list built outside the workshop, because every player would use it). SYNTH's action row gains SAVE PRESET ▸ between SCRAMBLE and SEND TO PAD: a slip with a placeholder name (`KICK 1`), the name rules judged live by `UserPresets.check` (blank, over fourteen letters, a factory name on this voice, or one of your own — which turns the button into REPLACE and says the old settings are not kept), and the save landing as a second strip, YOURS, under the factory row for that voice on every kit. One file, `presets.json` at the shelf root, every entry the same JSON a pad's recipe carries plus when it was saved; atomic writes, unknown entries carried, an unreadable file never written over. Promotion into the shipped roster stays a code change: SEND TO BENCH carries the file and renders each preset as its table's own `p(...)` line with exact floats, for the spread, blocklist and identity tests to judge | CORE + APP | M | a saved preset read back after a restart is equal and renders the same bytes; a factory name is refused and a saved name replaces in place; every roster line names a table and helper that exist, and every factory preset's own line reads back to its exact floats |
...
| X3.4 | ✓ done: BACKUP carries the player's presets (`docs/WORKSHOP.md`, WS5's last follow-up) — `presets.json` rides at the zip's root beside the `.xpn` kits when the shelf has one (`KitBackup.backup`'s `extras`, read back bounded by `KitBackup.extra`), both toasts count it, and a backup shared back in lands its kits first and then merges the presets into the shelf's own through `UserPresets.merge`: nothing on the shelf is written over, a twin is skipped (under its name or the fresh name an earlier landing gave it), a namesake with other settings lands under `NAME 2`, the bin rides minus what expired; a presets file that cannot be read or weighs too much still lands every kit and is named among the skips. CLI `backup` / `restore` do the same | CORE + APP | S | backup → bare shelf → every preset under YOURS and the bin with its days; the same backup again lands no preset twice; a shelf that saved its own namesake keeps it and gets the backup's as `NAME 2`; a garbage or oversized presets entry lands the kits and names the file |
...
| X4.8 | ✓ done: BENCH NOTES (`docs/WORKSHOP.md`, WS4) — the tester's own words, stamped by the phone. With the WORKSHOP open the title bar ends in a NOTE chip on every screen; a tap drops a slip whose context line is read at the tap (the screen, the open kit, the pad whose sheet is up, the time — `BenchNotes.Stamp`, read once so the line shown and the line written cannot disagree), and KEEP appends it as one line to `Bench/notes.jsonl` beside the kits. SEND TO BENCH carries the file and renders every note in its manifest as a `→` line ready to paste under the `docs/BENCH.md` row it answers; a shelf with nothing but notes still sends | CORE + APP | S | a note taken on PLAY names PLAY, the kit and the time in the manifest; a note with a pad sheet up names the pad; the bench zip still carries no audio |
| X4.7 | ✓ done: LABEL THIS HIT and SEND HITS TO BENCH (`docs/WORKSHOP.md`, WS3) — the calibration corpus's door from the phone. A pad sheet's BENCH box (WORKSHOP open) offers the corpus's nine classes as chips; a tap copies the pad's WAV into `Calibration/` beside the kits as `<label>_<kit>_<pad>.wav`, the harness's own naming rule (`LabelledHits`, whose test reads `CalibrationCorpusTest`'s source to hold the words and the rule); one label per pad, the lit chip takes it back out, a render (a recipe with a patch) refuses in words. SEND HITS TO BENCH on SETUP packs the folder as `SnipSnap Hits <stamp>.zip` under its own note — the one button that sends audio — so SEND TO BENCH's never-audio promise stays whole | CORE + APP | S | a hit labelled on the phone lands in `CalibrationCorpusTest`'s confusion matrix by drop-in; a render is refused; the two zips never mix |
| X4.6 | ✓ done: CONFIRM ALL and the cut rating (`docs/WORKSHOP.md`, WS2) — with the WORKSHOP open, CHOP grows a BENCH row: CONFIRM ALL vouches for every chip the classifier named and the human left alone (`ChopReviewModel.confirmAll`, logged as `TeachLog` lines whose label is the machine's own verdict — `Example.confirmation` — so the harness counts accuracy, not only misses; the picker's own agreement counts without it; a ghost's or a rung's LOOP never does), and five stars rate the cuts (`CutRatings`, `cuts.jsonl`: the bench's settings, tries before the chop, MERGE/SPLIT after it, chips corrected and confirmed). Both write at SEND with TEACH on, dim with it off; SEND TO BENCH carries both; `CutRatingsTest` sums the ratings by setting | CORE + APP | S–M | a confirmed-and-corrected chop replays with its accuracy counted; a rating line names the setting that earned it; the row is dim and says why with TEACH off |
...
folds in, so a swung ring now rides to the export intact. What was still
missing, at `a2ced3c` where this survey was taken, is *per-hit* time:
`OrbitHit` was `(step, slot, velocity)`, so there was nowhere to put the
seeded jitter `GrooveVariations.humanize` applies, nowhere to put the
sixteen distinct per-position offsets a `GrooveFeel` template carries,
and no swing at all on a ring whose step is not a 16th — which is exactly
the spanned, polyrhythmic ring ORBIT exists for. (YYY5 has since added
`OrbitHit.offset` and `OrbitFeel`; this paragraph is the baseline the
wave was planned against, kept in the past tense it belongs in rather
than rewritten, since it is the record of why these rows exist.)
...
| # | Work | Owner | Size | Exit test |
|---|---|---|---|---|
| YYY1 | ✓ done: empty-clip handling — and it turned out to be two different answers to the same input, correctly. The `GrooveVariations` half landed first in the circular-sequencer wave, and **not** the way this row proposed: `sparse` now returns a note-less clip unchanged instead of refusing, because `GrooveEdit.startEmpty` deliberately stores an empty base when GROOVE opens on a kit that has recorded nothing — there, an empty clip is a real state a player reaches by tapping STEPS, and its skeleton is itself. `standard`, `swing`, `halfTime`, `humanize` and `quantize` were already safe, and this pass verified that rather than assuming it. `Arranger.arrange` was the half left over, and it takes the opposite answer for the opposite reason: there is no song to plan out of silence, so it refuses by name above `Arranger.arrange`'s median arithmetic — `velocities[velocities.size / 2]`, named rather than numbered because this citation has now rotted twice under edits in this wave alone — which would otherwise raise a bare `IndexOutOfBoundsException` on the empty list. Both halves are the same contract — a typed refusal or a valid result, never a raw stack trace — and which one applies depends on whether the caller can mean anything by silence | CORE | S | ✓ a note-less base through `Arranger.arrange` raises a named refusal carrying the groove's name; proven by deleting the guard and watching the same test raise `IndexOutOfBoundsException: Empty list doesn't contain element at index 0`; one note is enough to plan from; and the six transforms a note-less base actually meets — `sparse`, `standard`, `swing`, `halfTime`, `humanize`, `quantize` — return rather than throw. Not the whole family: `fill` and `ghosted` still refuse by name when the kit has no snare, clap or perc to roll or whisper on, which is a precondition about the *kit*, not about the clip, and is the contract working rather than a gap in it |
| YYY2 | ✓ done: an empty export says which empty it is — and it took two goes, the first of which is the lesson. Implementation found a fourth shape beside the three this row listed (a set with no rings at all), so four refuse now, each with its own sentence: no rings, every ring a snip, no ring with a hit on it yet (what `+ PAD RING` makes before a step is tapped, and the one a single catch-all would explain worst), and every ring that does have a hit being muted. Those last two are asked in that order deliberately: asking "are the engaged rings empty?" first tells a set whose only played ring is muted that no ring has a hit on it, while one plainly does — so ask what exists before asking what is audible. The first attempt folded all four into the existing `refusal()`, which was a regression: `refusal()` is **shared**, and `OrbitScreen` gates both ways out on it — `bounceToTape()` and an OUT panel that hides both buttons — so a snip-only set was told `BOUNCE IT INSTEAD` with the bounce button hidden, naming the one action it had just disabled. Snips bounce fine; `OrbitEngine.addLoop` renders them. So the question is split rather than the check scoped, because the two callers really do share one of the two reasons. `refusal(set)` stays the ceiling both ways out share — one cycle of audio and one cycle of notes have the same 64-bar limit. `clipRefusal(set)` is `refusal(set) ?: noNotes(set)`, and only `clip()` and `clipIntoKit()` ask it. The OUT panel keeps BOUNCE on the row when only the clip is blocked and puts the reason where CLIP was. The snip rings a mixed set leaves behind stay a report, not a refusal: `OrbitClip.snipRings` names them and the success toast counts them | CORE | S | ✓ all four shapes refuse from `clipRefusal()` with a distinct reason and throw from `clip()`, while `refusal()` stays null for every one of them so the bounce survives; the ceiling reaches both doors unchanged; a mixed set still clips its pattern rings and reports the snips; and the invariant, pinned across eight sets: whenever **`clipRefusal()`** returns null, `clip()` carries at least one note — never a silent empty clip saved into `groove.json`, where it would become the kit's base and read as a kit whose beat is nothing. The `OrbitScreen` wiring is the part no JVM test covers; CI's `android-build` compiles it |
| YYY3 | ✓ done: one pad-to-note map, and it was never a choice between losing notes and refusing. The clamp in `noteFor` (`(35 + slot).coerceIn(0, 127)`) was simply the wrong operator: `Mpc3TrackWriter.padNoteMap` writes `(36 + padIndex) % 128` — "chromatic with wraparound", as its own doc says — so pad 93 is note 0 and pad 128 is note 35, and the map is **bijective over all 128 slots**. Clamping sent every slot from 92 up to note 127, which failed twice over: the notes deduped against each other, and whatever survived addressed pad 92 on the hardware. So the map now lives once, on `Mpc3Note.noteFor`/`slotFor` beside the writer whose convention it is, pinned by a test against `padNoteMap`'s own arithmetic — nothing else in the build checked that those two agreed, which is how a clamp sat in one of them. Both inverses had to come along, and that is the part worth noting. `KitPreview`'s was `note - 36 + 1` and GROOVE's live roll called `hit(note - 35)` — each right only above the wrap. Fixing the forward map alone would have been the first thing ever to write notes 0..35 into a clip, so it would have *introduced* two silent failures rather than found any: a clip that plays on the MPC and renders in a preview while being silent in the app's own roll. Review found the second one; the first had already taught the lesson and I still swept for producers without sweeping for readers. The cross-kit half is a refusal, as this row proposed: a clip rides a track, a track carries one program, and `save` writes into one kit's `groove.json`, so a two-kit set has no single-clip reading to honour. It names both kits. A pad past 128 is the same question at the other end and refuses by number | CORE | S | ✓ the map is bijective over 1..128 and matches `padNoteMap` slot for slot; slots 1..91 keep the exact notes they always had, so every clip written before reads the same; two pads above the wrap struck together export as two notes addressing the pads actually played; a two-kit set refuses naming both while still bouncing; a pad past 128 refuses by number; a kit with a pad above the wrap renders in `KitPreview` instead of silently skipping — each proven by reverting the guard and watching the matching test fail |
| YYY11 | ✓ done: every other caller of the pad-to-note map, migrated with YYY3 rather than after it. The row first named four — `GrooveVariations.fill`/`.ghosted`, `CapturedGroove.clip`, `KitPreview.defaultPattern` — and review named the rest: `ReadGroove`, `EuclidCommand`, `LearnCommand`, GROOVE's live-record producer, and `GrooveEdit.noteFor`, plus two readers — GROOVE's roll, and `Chart.slotOf`, which review found after the first sweep called the inventory complete. Nine producers and three readers, each with its own copy of the arithmetic. `Chart` is the one worth recording: it spelled the map as a named `NOTE_OFFSET = 35` rather than a literal, so a grep for the digits missed it — searching for the spelling instead of the concept is how a sweep declares itself finished one caller early. A producer with a bare `35 + slot` passes `Mpc3Note`'s `require(note in 0..127)` only while the slot stays under 93, and `XpnImporter`/`SfzImporter` both accept slots to 128 — so importing an `.xpn` or `.sfz` that uses the upper banks and then previewing, filling or ghosting it threw `note out of range: 128`. Verified before the migration and again after. Deferring these was the wrong call and review said so plainly: a map nine callers ignore is not canonical, and the reader in GROOVE's roll was not a deferral at all but a regression YYY3 would have shipped. `Chart.slotOf` was the third reader: GROOVE's CHART button renders through it, so a saved clip with a wrapped note charted the pad as a negative slot — `NOTE 0 (NO PAD)` for a pad the kit really has. With the map bijective there is no such thing as a note with no pad, so that whole branch is gone, and a chart now names the pad the hardware would play. `Mpc3TrackWriter.padNoteMap` now derives its own entries from the same helper, so the arithmetic exists once in the repository rather than twice agreeing | CORE | S | ✓ a kit with pads at 93, 100 and 128 fills, ghosts, previews and captures instead of throwing; the step editor's lanes return exactly the notes they always did; the writer's serialised `padNoteMap` is asserted against the helper entry by entry, so a drift between them fails rather than silently addressing the wrong pads; a wrapped note charts as its own pad instead of `NO PAD`; each proven by reverting a call and watching the failure come back |
| YYY4 | ✓ done: choke in `OrbitEngine`. `KitPreview.render` honoured `pad.muteGroup` with a ramped end and the hardware honours `whichMuteGroup`, but the live engine had no reference to `muteGroup` anywhere and `OrbitBank.pad()` handed back a bare `Snip` with no pad metadata — choke was structurally absent, so an open hat rang through a closed hat in ORBIT while choking correctly in the preview, in the mixdown and on the MPC: one pattern, three different sounds. The metadata turned out to be already in hand and thrown away — `KitSampleSource.indexKit` loads the whole `Kit` and kept only `slot -> sampleFile`. So `SampleSource` gains a defaulted `muteGroups` (defaulted precisely so the eight test fakes and both bakers need not implement it at all — only the two production sources override, and an empty map means exactly "as before"; and per kit rather than per pad, because one `prepare` wants every pad's group at once and answering a slot at a time makes the source re-read the kit once per pad), `OrbitBank` carries the group beside the audio and re-reads it on every prepare so retuning a kit's choke reaches the next block, and `OrbitEngine` ends the ringing voices in a group when a new one starts. One thing the row did not anticipate: choke means *the newest hit wins*, and rings are sorted within themselves but not against each other — ring two's downbeat can fall before ring one's third step — so hits in a block now start in merged time order. Without it a later ring's hit is started first and the earlier one chokes it, which reads as no choke at all. The fade is `AutoPlace.CHOKE_FADE`, moved beside the mute group it belongs to rather than left as the same 128 written in two files that must agree — the duplication YYY3 existed to remove. Review then found two more, both real: a choke arriving inside the last fade's worth of a pad left the stop point where it already was, so "was it shortened?" answered no for a voice that really had been choked and it ran out at full level — the ramp is now tracked rather than inferred, and normalised over whatever is left, because a fixed-length slope applied to a short tail starts part-way down and clicks; and `KitSampleSource` cached the group beside the filenames it keeps for the life of the source, which would have meant toggling choke in the pad sheet did nothing in ORBIT until the app restarted — the group is now re-read when `kit.json` has been written since, which the bake path's caches cannot afford but `prepare`, being off-thread, can. A second round found three more, all real: a group number is scoped to its program, and `AutoPlace` puts every kit's hats in the same one, so a set with rings from two kits had one kit's hats silencing the other's — the voice carries its kit now; a hit arriving while a voice was already fading re-derived that voice's ramp from the new instant, setting its gain back to full so it jumped *up* mid-fade, which is the click the ramp exists to prevent — a voice already on its way out keeps the ramp it is on; and `KitPreview` had the same near-end bug the engine did, clamping the end with `min` so a choke inside the last fade left it unshortened and skipped the ramp entirely — fixed there too, because two renderers that disagree is the condition this row exists to end | CORE + APP | S | ✓ two hits in one mute group leave one voice sounding, not two summed; no group, or different groups, ring on exactly as before; a choked voice ramps rather than cuts, and a voice that merely reaches the end of its sample keeps the tail it was recorded with; a pad chokes its own previous hit; a disengaged ring chokes nothing; the group is read off `kit.json` through the real `KitSampleSource`; a source that says nothing about choke plays identically — each proven by reverting the guard and watching the named test fail. Bench: ORBIT's hats choke under a finger |
| YYY5 | ✓ done: per-hit time on `OrbitHit`. The set-level `swing` works and reaches the export, but one number is not sixteen: `GrooveVariations.humanize`'s seeded jitter had nowhere to live, a `GrooveFeel` template is sixteen distinct per-position offsets and so could not reach rings at all, and a spanned ring had no pocket of any kind. `OrbitHit` gains `offset`, in **pulses** rather than frames — a set outlives the device it was made on, and frames would shift the whole pocket when the same file opened at another sample rate — bounded to a 16th either way, because past that a hit reads as belonging to a different step and the step is what the ring draws. `OrbitFeel` is the new writer of it: `apply` lays a donor's sixteen offsets on, `humanize` seeds a take, `straighten` puts everything back. A hit takes the offset of the sixteenth it LANDS on, derived from its grid place in frames rather than its step index, so a three-step ring across a bar takes positions 0, 5 and 11 — that is how the polyrhythmic ring gets a pocket. Deliberately the grid place and not the sounding frame: swing displaces a hit, it does not renumber it, and reading the position off the swung frame puts step 1 at swing 75 on the third sixteenth's pocket. The two swings are now one arithmetic: `Mpc3Clip.swingPush` rounds to nearest where the groove side truncated through `Long` division, so a ring and a clip at the same percent write the same pulse — verified across the whole 50..75 ladder, and it moves every exported groove's swung note by one pulse, which is the point. The engine keeps the exact float ratio rather than the rounded pulse, and the export no longer reaches it at all: review found that routing the clip through frames was itself the bug. `sampleRate` is only required to be positive, and once a frame is coarser than a pulse — fewer than 960 frames to a beat, which is a rate below 16 × BPM hertz, and 832 Hz at 120 BPM gives 416 — rounding to a frame and back moved the note, so a ring at swing 58 exported 279 pulses where the clip wrote 278. The export now counts in pulses throughout (`periodPulses`, `stepPulses`, `firingPulses`, sharing one lap walk with `firings` so the two cannot disagree about which laps a hit lands on), which makes a hit's `offset` pass through untouched rather than being converted out of the unit it was stored in and back. A clip is musical time: the same set exports the same pulses at 1 Hz and at 192 kHz, and the change is a no-op at every rate anyone plays at — the whole suite passed unaltered. **The row's fourth ask is declined with a reason**: `swingFrames` still gates on `stepIsSixteenth`, because MPC swing is defined on pairs of 16ths and a three-step ring across a bar has none; pushing its odd steps by 0.32 of a 16th is a 6% nudge with no musical meaning, and one control would silently mean two different things depending on span. Per-hit offsets give those rings a pocket properly, which is what the row actually wanted | CORE | M | ✓ a hit leans both ways and reaches the export in pulses; a lean past a 16th is refused; a hit dragged before the downbeat sounds at the end of the lap before; a donor's sixteen offsets land on a ring and on a spanned ring's real positions; a donor-silent position leaves the hit straight; swing does not renumber a position at any point on the ladder; a humanised set re-renders identically for one seed and differently for another, and exports the same file; a ring and a clip at the same swing percent land on the same pulse at every rate from 832 Hz up, not merely at 48 kHz, and one set exports one clip whether it is playing at 1 Hz or 192 kHz; `orbits.json` is version 4, versions 1–3 still load, and a straight hit writes no `offset` key — so a hit nobody has given a pocket keeps the exact shape it always had. Not the file's bytes: `version` itself goes 3 to 4, so a version 4 file is never byte-identical to what an older build wrote. The guarantee is about hits, not files, and I had it loosely worded in five places — the store's write comment, a test comment, this row, and twice in the PR body — before review caught the first and re-reading my own claims caught the rest; a fractional or out-of-range `offset` in a file refuses through the strict whole-number accessor rather than truncating to 37 or clamping to Long.MAX_VALUE; and `positionOf` agrees with `GrooveFeel`'s own quantiser on an exact tie, the four-step HALF ring in a twelve-step bar, asked against the donor path itself rather than a second copy of its arithmetic — each proven by reverting that guard alone |
| YYY6 | ✓ done: grooves → rings, the missing direction. `OrbitImport.rings` is the inverse `OrbitClip` never had, and every source reaches it because every source already funnels into `Mpc3Clip` — a captured break, an imported `.mid`, PROG E and the chop pipeline's grooves are one function away each. **The round trip is exact, not “within a 16th” as this row assumed**: a note's pulse becomes the 16th it is *nearest* plus its lean off *where that step fires*, so a clip goes out and comes back on the pulse it started on. The lean is measured from the firing pulse rather than the grid because `stepPulses` adds the set's swing, and review found the first cut exact only into a straight set — joining a swung one gave every odd step the set's push on top of its recorded time, moving material the import exists to leave alone. The destination's swing is therefore an argument. Two notes the clip legally holds on one pad at one pulse are deduped on the way in by `GrooveEdit.dedupeLouder`, the export's own rule rather than a second copy of it, and counted: carrying both in would put two hits where only one can come back, losing a note the import had promised to carry. That was impossible when the row was written and is the reason YYY5 was its prerequisite — without `OrbitHit.offset` an off-grid note could only be snapped to the grid and its feel thrown away. Nearest rather than the step it is past, because a note dragged early belongs to the beat it is leaning into; a note in the last half-16th therefore rounds forward to step 0 carrying a negative lean, which is a pickup, and `firings` already sounds those on the lap before — so even the wrap round-trips. **Two deviations from the row, both deliberate.** The row said a clip that will not fit `MAX_ORBITS` refuses. Refusing loses a whole break over its ninth pad, and a kick, a snare, two hats, a clap, a rim and two toms is eight before anything unusual happens; so the surplus pads share the last ring instead, which drops nothing — `Orbit.voice` holds as many pads as it likes, exactly as `OrbitPresets.bassRing` puts a whole scale on one circle — and the player pulls them apart when there is room. The pads with the most hits keep their own circle, since those are the ones worth re-lengthing. The `MAX_STEPS` half of that refusal stands as written: five bars is 80 sixteenths, a ring holds 64, and there is no way to take it that is not dropping material. The row was also marked CORE, but a core function nothing calls makes nothing feel like one instrument, so ORBIT's OUT panel — where CLIP ▸ KIT already explains the route to the grooves — gains GROOVE ▸ RINGS, the return leg. It **joins** rather than replaces: a break next to a ring that drifts against it is what ORBIT is for, where a break that cleared the screen to arrive would just be GROOVE drawn round. The ring budget is therefore the caller's, not the constant, since an import joining six rings has two. A note whose pad the kit lacks is skipped and named, matching what `OrbitBank` already does at play time with a `(kit, slot)` it cannot find; a clip naming no pad the kit has refuses rather than opening empty, since there is nothing to open. The screen prefers PROG E over the captured base — `GrooveStore.load` hands back oldest first, which is always the base, so the first cut could never reach the program the player had actually been editing — and revalidates the set before committing, since the budget was measured before the file was read and `OrbitSet` refuses a ninth ring by throwing, outside any `runCatching` | CORE + APP | M | ✓ a clip through rings and back is note-for-note identical — pad, pulse and velocity, not merely the same 16th — with every note deliberately off the grid; all 128 notes land on the pad `slotFor` names and return as themselves, note 35 and the upper banks included; a note in the last half-16th becomes step 0 with a negative lean and still round-trips; each pad gets its own ring named after the pad, every ring the clip's own length; a clip lands where it was recorded at every swing on the ladder, because the lean is measured from the firing pulse and not the grid — and that lean reaches a whole `MAX_OFFSET` where the destination swings, never more, swept across every pulse of a bar; a pickup and the downbeat it wraps onto keep both their moments and the count of shared squares is reported, since the grid draws one square per step and pad; nine pads into eight rings share rather than lose one, and a smaller budget shares sooner; a missing pad is named and the rest still opens; five bars refuses by its length while four bars is exactly a ring; a clip with no notes refuses by name — each proven by reverting that guard alone, and the two that matter are the lean (drop it and the round trip fails) and nearest-step rounding (floor it and only the pickup fails, since floor still round-trips exactly — the difference is musical, not arithmetic). The `OrbitScreen` wiring is the part no JVM test covers; CI's `android-build` compiles it |
| YYY7 | ✓ done: probability, conditionals and ratchets. The row said `OrbitHit` carried "velocity and nothing else" — stale by two rows, since YYY5 gave it an `offset` and YYY8 a `length` — and it said the roll arithmetic already existed in `GrooveVariations.fill`, which is a 32nd roll hard-coded to beat 4 of a fill bar with its own velocity ramp, a reference rather than anything callable. What landed is four fields on `OrbitHit` (`chance`, `everyLaps`/`onLap`, `ratchet`) and a `seed` on `OrbitSet`. **The roll is not random.** `OrbitClock.sounds` is a pure function of the seed, the ring's own identity, the hit's identity and the lap number, so a set renders to the same audio twice and the engine and the export agree lap for lap — a live `Random` would have made a bounce that did not match what was heard, and a rewind a different take. Two things the row did not foresee. **A conditional changes the cycle**: a hit on one lap in four does not repeat until the fourth lap, so `OrbitClock.turnSteps` multiplies its ring's length before the LCM, the clip written from it is four times as long, and `OrbitClip.refusal` now names the conditional as a thing to undo instead of only saying "shorten a ring". A `chance` is deliberately **not** in that sum: a rolled hit has no period at all, so the export writes the rolls of the first cycle rather than pretending to one. **And the grid had no gesture left** — tap places, long-press cycled weight — so the APP side is `OrbitBrush`, a chip above the strip saying what the next long-press writes. It lives in `:loop` rather than `:app` because `:app` has no JVM test, and "what does the second long-press do" deserves an answer that runs. Exit: a seeded set renders identically twice and differently across laps ✓; a 1-in-4 hit fires on the laps its seed names ✓; a ratcheted hit exports as its sub-16th notes ✓; every new field round-trips through `orbits.json` (version 6) ✓. One limit found and left: at a sample rate coarser than a pulse, a hit leaning before the clip's own pulse 0 is played by the engine and truncated by the export — mechanically verified as the merged base's behaviour, older than this row and untouched by it. | CORE + APP | S | done |
| YYY8 | ✓ done: note length. The row's finding reproduced exactly — every exported note was 240 pulses, the bass ring's twenty-eight included — while the engine played each sample out with nothing but a choke able to stop it: one pattern, two ideas of when a note ends. `OrbitHit` gains `length` in pulses, with `WHOLE_SAMPLE` (0) meaning play the sample out. Zero is the default precisely so nothing changes for a set made before it existed: a kick is over when the kick is over, and gating every drum at a 16th would be a new and worse sound. A gated hit ends live over the choke's own ramp rather than a fifth fade length of its own — the two are the same act, a voice cut before its sample ran out, and a sample stopped mid-cycle clicks whichever reason stopped it. They share the code as well as the idea now (`OrbitEngine.endAt`), because writing the ramp twice got it wrong once: the gate clamped its fade against the stop point instead of against what remained of the sample, so a pad ten frames longer than its gate ran off its own buffer and threw. And the flag that switches the ramp on had to stop doubling as the choke's “already leaving” guard: a voice whose end is merely scheduled sits at full gain until it gets there, so a gated hit rang straight through the hit that should have silenced it — YYY4's rule broken by YYY8's. The guard asks where the ramp starts (`at >= limit - fadeLen`) rather than whether a flag is set, which is also the plain “has it finished?” for a voice nothing has shortened. `length` is bounded to the 64 bars both outputs already cap a cycle at, because `orbits.json` carries numbers as JSON Doubles and a value past 2^53 would round on the way out and read back as a different note. The export writes a gated hit's own `lengthPulses` and an ungated one's 16th, so a set with no lengths exports byte-for-byte what it did. `orbits.json` is version 5, writing `length` only where there is one. `OrbitClock.framesForPulses` is now the single pulses→frames conversion, with `offsetFrames` delegating to it: a lean and a length are the same question, and two answers to it drift. And YYY6's import carries a clip's `lengthPulses` through, which completes the round trip it opened — that direction had been exact in time and silent about duration. **What the row could not know**: its exit line asks that the offline render and the export agree on where a note stops, and they cannot be made to in general. docs/MPC3_FORMAT.md records `triggerMode` from the corpus as One Shot (the whole sample fires and ends), Note Off (plays while held) and Note On (vocals, loops, sustained chops) — but `KitPad.oneShot` collapses those three into a boolean, so nothing in the model can say whether a given pad reads a note's length at all. Both sides are therefore told the same number and that is as far as agreement can be carried honestly; making the engine consult `oneShot` would gate Note On pads live on a guess about hardware this repo has not tested. `OrbitPresets.bassRing` is left straight for the same reason: its tonal pads are one-shot by default, so a length there would gate the voice live while the export wrote a number the hardware's One Shot mode does not read — the disagreement this row exists to remove, pointing the other way | CORE | S–M | ✓ a hit with no length plays its sample out; a gated hit ends at its own length and ramps rather than cutting; a gate measures sample played, not where in the block the hit landed; a length longer than the sample leaves the voice alone, including one whose frame count overflows `Int` at a legal rate; a length outside 0..MAX_LENGTH is refused at both ends, and MAX_LENGTH itself survives the file exactly; a gated hit exports its own length while an ungated one exports a 16th, and a set with no lengths exports what it always did; a length round-trips through `orbits.json` while a version 4 file loads with every hit playing out; a clip's note length reaches the ring and goes back out as it came in; and one conversion answers both a lean and a length — each proven by reverting that guard alone. Two of those reverts earned their keep: dropping the ramp flag proved the gate would otherwise stop flat, and the block-position revert passed, which is how a real bug was found — the first cut added the voice's negative block position to a sample index and gated a step-1 hit nearly two thousand frames early, while the test as written accepted both answers |
| YYY9 | ✓ done: ORBIT sections. Every claim in the row checked first and all of them held — `Arrangement`, `BeatTape.arrange` and `SessionBuilder` are where it said, rings had neither, and `OrbitClip` did already compute the cycle. `OrbitSection` names how long it lasts in the set's own bars and which rings play; an empty set of rings is a **break**, deliberately not read as "all of them", since a default that means the opposite of what it says is how a drop becomes a full bar on stage. **The decision the row turns on: a section RESTARTS its rings**, and it follows from the row's own exit criteria rather than from taste. A section as a mute lane over a set still turning underneath fails both: an arrangement has to *repeat*, which under that reading happens only when the section total and the rings' cycle agree; and the exported clip has to be true every time the hardware flips to it, which under that reading it is not — the rings would be elsewhere on the second pass. So the clock's one number is counted from where the section began (`OrbitClock.localFrame`), and the drift is unhurt: inside a section the rings are the same pure function of the same number they always were. **The export half was free**, the YYY6 shape again: `ExportFormats` already turns every stored groove into its own sequence — "the pattern flip on the hardware's switcher" — so the missing piece was never the export, it was having more than one clip to give it. `OrbitClip.save` now writes one clip per section, appended, so an arrangement never becomes the kit's base. **What the row could not know**: the order does not ride along. `Mpc3Song`'s step schema (which sequence, how many repeats) has never been captured from the corpus, so `Mpc3ProjectWriter` refuses a non-empty song (GG3.2) and the sections arrive as sequences a player flips by hand. That is why they keep their names — the name is the only thing left saying what order they were in. A break writes no clip at all: there is nothing to flip *to* for silence, and a note-less clip is what `noNotes` exists to keep out of `groove.json`. Exit: a two-section set renders each section for its length and repeats ✓; the export carries the sections as the sequences the switcher flips ✓ (the switcher, not the song); a set with NO arrangement exports byte-identically to today ✓, held by a test comparing whole `Mpc3Clip`s, name included. A set with ONE section is deliberately not that case — review caught the claim, correctly: `Mpc3Clip.name` is serialised and a section's clip takes the section's name, because on the hardware that name is the only thing left saying which sequence is which. | CORE + APP | M–L | done |
| YYY10 | **implementation complete, hardware-unverified** — not ✓ done, because the acceptance evidence does not exist yet: meter, at the project level — and the probe found it was not the metadata question the row asked. A 3/4 set's clip was written into 4/4 bars: `OrbitClip` rounded a 24-step section up to two bars of 3840 and `endPulses` followed, so the exported loop played eight sixteenths of silence the set never plays. Declaring the meter is what sizes the container to the music, so this shipped as a fix rather than an annotation. `Mpc3Clip` gains `pulsesPerBar` (defaulted, so every donor groove, import and step-editor clip is untouched); a project sequence declares the meter and takes `timeSignatureTrack`, `lengthBars`/`lengthPulses`, the loop bounds and each clip's `endPulses` from that one number, with the corpus's own cross-check asserted on our own output; two meters in one sequence is refused. **Deliberately not done:** the track path still pads to 4/4, because its clips are version 1 and `timeSignatureList` is a version-3 field — writing it there would invent a shape no real file has; `PULSES_PER_BAR` stays 3840 and `GrooveFeel.POSITIONS` stays 16, since donor grooves really are 4/4. One guard the probe did not predict: `lapSteps` is only `1..MAX_STEPS` and `OrbitStore` reads it from JSON, so a 13-step lap is reachable and cannot be spelled as whole quarters — those keep their 4/4 container instead of throwing. **Outstanding, and it is the important part:** no corpus file carries a `beatsPerBar` other than 4, so the first non-4/4 `.xpj` is the first a Live III has been asked to read, and the failure mode if it refuses is worse than the padding this fixes. One bench check settles it — export a 3/4 set, open it on the device | CORE | M | the writer half is in with the suite green; the row stays OPEN until a 3/4 `.xpj` has been opened on a Live III, which is the one piece of evidence `reference/` could never supply |
...
| ZZZ3 | ✓ done: VINYL — `Vinyl`, a rack section between DUB and TAPE: rumble, groove hiss and seeded clicks (CRACKLE, RUMBLE, HISS), each bed scaled to the input's own peak before summing and capped like `TapeWear`'s own ceilings, then peak matched; the `vinyl` character | CORE | S | all-zero macros return the input bit for bit; the same seed crackles the same way twice; a kick and a snare through VINYL still classify; silence in stays silence, never a bed with nothing to scale against |
...
| ZZZ8 | ✓ done: the pad sheet regrouped — six rows in place of five, grouped by what an effect *does* rather than which family implements it: row one the eras plus SMEAR (CRUSH · TAPE · DIRT · SMEAR), row two the hit's anatomy (SWELL · TAIL · SKIM · GHOST · SPIKE), row three its character once it is itself (PUNCH · RING · DUB · VINYL · PHASE), row four what happens to it in time (SLAP · WASH · ROLL · GATE — ROLL and GATE keyed treatments sitting here because that's what they sound like, the way TUNE has always sat among the characters), row five the machine's transport (FLIP · STOP · START · PITCH), row six the keyed family (TUNE · BODY · WOBBLE · ETERNAL) | APP | S | 27 chips across six rows, no duplicates; every segment names a real treatment; the inverse mappings hold |
