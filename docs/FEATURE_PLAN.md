...
| # | Work | Owner | Size | Exit test |
|---|---|---|---|---|
| YYY1 | ✓ done: empty-clip handling — and it turned out to be two different answers to the same input, correctly. The `GrooveVariations` half landed first in the circular-sequencer wave, and **not** the way this row proposed: `sparse` now returns a note-less clip unchanged instead of refusing, because `GrooveEdit.startEmpty` deliberately stores an empty base when GROOVE opens on a kit that has recorded nothing — there, an empty clip is a real state a player reaches by tapping STEPS, and its skeleton is itself. `standard`, `swing`, `halfTime`, `humanize` and `quantize` were already safe; now verified so. `Arranger.arrange` was the half left over, and it takes the opposite answer for the opposite reason: there is no song to plan out of silence, so it refuses by name one line above the median arithmetic (`Arranger.kt:79-80`) that would otherwise raise a bare `IndexOutOfBoundsException` on the empty list. Both halves are the same contract — a typed refusal or a valid result, never a raw stack trace — and which one applies depends on whether the caller can mean anything by silence | CORE | S | ✓ a note-less base through `Arranger.arrange` raises a named refusal carrying the groove's name; proven by deleting the guard and watching the same test raise `IndexOutOfBoundsException: Empty list doesn't contain element at index 0`; one note is enough to plan from; the whole `GrooveVariations` family returns rather than throws |
| YYY2 | ✓ done: `OrbitClip.refusal()` names an empty export. Implementation found a fourth shape beside the three this row listed — a set with no rings at all — so four now refuse, each with its own sentence, because they are four different mistakes: no rings, every ring a snip, every pattern ring muted, and an engaged ring carrying no hits (what `+ PAD RING` makes before a step is tapped, and the one a single catch-all refusal would explain worst). The CLIP ▸ KIT path needed no change to surface any of it: `clipIntoKit()` already toasts whatever `refusal()` returns, so widening the refusal was the whole fix. The snip rings a mixed set leaves behind are a report rather than a refusal, so `OrbitClip.snipRings` names them and the success toast counts them — that one line in `OrbitScreen.kt` is the only part of this row no JVM test covers | CORE | S | ✓ all four shapes refuse with a distinct reason and throw from `clip()`; a mixed set still clips its pattern rings and reports the snips left behind; and the invariant the row exists for, pinned across eight sets: whenever `refusal()` returns null, `clip()` carries at least one note — never a silent empty clip saved into `groove.json`, where it would become the kit's base and read as a kit whose beat is nothing |
