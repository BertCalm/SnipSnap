...
| YYY3 | ✓ done: one pad-to-note map, and it was never a choice between losing notes and refusing. The clamp in `noteFor` (`(35 + slot).coerceIn(0, 127)`) was simply the wrong operator: `Mpc3TrackWriter.padNoteMap` writes `(36 + padIndex) % 128` — "chromatic with wraparound", as its own doc says — so pad 93 is note 0 and pad 128 is note 35, and the map is **bijective over all 128 slots**. Clamping sent every slot from 92 up to note 127, which failed twice over: the notes deduped against each other, and whatever survived addressed pad 92 on the hardware. So the map now lives once, on `Mpc3Note.noteFor`/`slotFor` beside the writer whose convention it is, pinned by a test against `padNoteMap`'s own arithmetic — nothing else in the build checked that those two agreed, which is how a clamp sat in one of them. `KitPreview` had to come along, and that is the part worth noting: its inverse was `note - 36 + 1`, right only above the wrap, and fixing the forward map alone would have been the first thing ever to write notes 0..35 into a clip — so it would have *introduced* a silent render failure rather than found one. The cross-kit half is a refusal, as this row proposed: a clip rides a track, a track carries one program, and `save` writes into one kit's `groove.json`, so a two-kit set has no single-clip reading to honour. It names both kits. A pad past 128 is the same question at the other end and refuses by number | CORE | S | ✓ the map is bijective over 1..128 and matches `padNoteMap` slot for slot; slots 1..91 keep the exact notes they always had, so every clip written before reads the same; two pads above the wrap struck together export as two notes addressing the pads actually played; a two-kit set refuses naming both while still bouncing; a pad past 128 refuses by number; a kit with a pad above the wrap renders in `KitPreview` instead of silently skipping — each proven by reverting the guard and watching the matching test fail |
| YYY11 | The other four callers of the pad-to-note map — found while centralising it in YYY3, and a live crash rather than a tidy-up. `GrooveVariations.fill`, `GrooveVariations.ghosted`, `CapturedGroove.clip` and `KitPreview.defaultPattern` each build the note as a bare `35 + slot`, which passes `Mpc3Note`'s `require(note in 0..127)` only while the slot is under 93. Verified: all four throw `note out of range: 128` on a kit whose snare sits at slot 93. That is reachable, not theoretical — `XpnImporter` and `SfzImporter` both accept slots 1..128, so importing an `.xpn` or `.sfz` that uses the upper banks and then previewing, filling or ghosting it raises a bare `IllegalArgumentException`. YYY3 left them deliberately rather than widen its own diff, but they are four one-line changes now that `Mpc3Note.noteFor` exists, and leaving them means a correct shared map that four callers ignore | CORE | S | all four take the shared map; a kit with a pad above the wrap previews, fills and ghosts instead of throwing; each proven by reverting the call and watching the same test raise `note out of range` |
