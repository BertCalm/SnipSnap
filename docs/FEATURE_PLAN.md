...
| YYY6 | Grooves → rings, the missing direction — `OrbitClip` runs one way, so a captured break, an imported `.mid` and PROG E can none of them be played by the live engine, and the chop pipeline's best material is invisible to ORBIT. Pulses → steps is arithmetic; the real decision is how one clip becomes several rings, and per-pad is both the obvious rule and exactly what `OrbitPresets` already builds. This is the single change that would make the three sequencers feel like one instrument. YYY3 removed this row's hard part rather than merely preceding it. The inverse used to be written by hand as `slot = note − 35`, which gave every note below 36 a slot under 1 for `OrbitHit`'s `require(slot >= 1)` to refuse — GM note 35 (Acoustic Bass Drum) being the common case, not a hostile one — while the old clamp at the top end left note 127 recoverable from any of slots 92..128, so no inverse could recover it at all. The map is bijective, so `Mpc3Note.slotFor` now recovers exactly one pad for every note in 0..127 and neither end has a hole. What is left is not arithmetic: a note whose pad the kit simply lacks. `OrbitBank` already skips a `(kit, slot)` it cannot find, so the policy should match it — skip and report, never a silent drop and never a whole-file refusal | CORE | M | a captured groove → rings → `OrbitClip.clip()` round-trips every note's pad, step and velocity within a 16th; a `.mid` opens in ORBIT as rings with every note landing on the pad `slotFor` names, note 35 and the upper banks included; one carrying a note whose pad the kit lacks opens with the rest and names what it left out; a clip that won't fit `MAX_STEPS`/`MAX_ORBITS` refuses with a reason rather than truncating |
