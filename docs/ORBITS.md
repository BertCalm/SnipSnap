...
| `loop/OrbitStore.kt` | `orbits.json` (version 5; versions 1–4 still load — a v1/v2 lock becomes a one-bar span, a hit with no `offset` sits on its step, and a hit with no `length` plays its sample out), a sidecar beside the kit like `groove.json` |
