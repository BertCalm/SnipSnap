...
| `loop/OrbitClip.kt` | The set as MPC clips in `groove.json` — one cycle where there is no arrangement, one per section where there is, counted in the clip's own 4/4 bars whatever the set's bar, and the 64-bar refusal both outputs share |
