...
**This is why the convention is broken twelve ways on one screen. The
...
by seven different actions; `remember(slot)` on a pinned navigation button
...
| **J9** | **Every KIT pad is silently dead if the audio engine didn't start — and still glows as though it played.** `hit()` returns on `!engineUp \|\| !player.isUp()` (`KitScreen.kt:189-199`); `engineUp` is never surfaced anywhere. The glow animation fires *before* `hit` (`:857-860`), unconditionally. So all sixteen pads animate correctly and produce silence, with nothing on screen saying the engine is down. | `KitScreen.kt:189-199`, `:857-860`, `:153-173` |
...
| **J27** | **CHOP's CUT box goes dead twelve controls at a time, without dimming.** Nine segments wrap themselves in `if (!busy)`; four more call through to `rechopTo`, which returns on `rechopBusy \|\| sendBusy \|\| humming` (`:432`). No dim is possible — `SegmentButton` has no `enabled` — and `:1533` announces `selected` regardless. Prior #9, confirmed and wider. | `ChopScreen.kt:1299-1307`, `:1338-1368`, `:432`, `:1521-1541` |
...
| **J31** | **CHOP: `SEND TO GRID` creates a kit whose name the user never sees or chooses** — while the smaller button one row below names both its destination kit and bank, in the label *and* the toast. The bigger, more permanent action is the anonymous one. | `ChopScreen.kt:981`, `:992-994`, `:1038`; `Personality.kt:1156-1157` |
...
| **J47** | Carried forward, still open: BOUNCE has three names for one destination (#25); CATCH is both the capture verb and a gesture (#27); group-box legends are repeated verbatim as card titles in all four boxes (#30). | prior #25, #27, #30 |
...
`DeckButton` have no `enabled` parameter, so twelve controls on that very
screen have nothing to dim *with* and no toast. The rule was written as a
