...
**This is why the convention is broken up to eighteen ways in one box. The
...
J5, J6, J7). Two of the five are **irreversible writes to disk** — J2
and J3, the two halves of the export overwrite — and neither carries a
*durable* warning: J2's is a one-shot toast on an arm nothing renders,
and J3 has none at any point. The other three (J5, J6, J7) destroy
in-memory work: corrected chips, a hum in flight, and a live CATCH
session.

A third irreversible disk write, `MAKE PAD ▸`, is catalogued in the
irreversibility section below rather than in the S1 table, so it is
...
| **J2** | **EXPORT's overwrite arm is invisible and never expires.** `session.overwriting` is the armed state in which the next tap destroys an existing export. There *is* a two-tap confirm: the first tap writes with `overwrite = false`, gets `WouldOverwrite` back, arms `session.overwriting` and toasts the doomed path **by name** (`:452-460`). The defect is that the arm is invisible and permanent. No *rendering* path reads it — the only read is inside the `startWrite` closure (`:452`) — so it changes no label, no enabled state and no semantics. `WRITE KIT` reads identically armed and unarmed; the sole warning is the transient toast already gone. The session is hoisted into `App` (`App.kt:535`), so the arm survives tab switches and remounts indefinitely. Tap, read the toast, come back an hour later, tap again — the export is replaced with no further warning. | `ExportScreen.kt:133`, `:452`, `:459-460`, `:463`, `:495`, `:617`, `:629` |
...
| **J9** | **Every *filled* KIT pad is silently dead if the audio engine didn't start — and still glows as though it played.** `hit()` returns on `!engineUp \|\| !player.isUp()` (`KitScreen.kt:189-199`); `engineUp` is never surfaced anywhere. For a filled pad `onTap` fires first (`:856`) and the glow coroutine launches on the very next line (`:857-860`), unconditionally and without ever reading `hit`'s result — so the pad animates correctly and produces silence, with nothing saying the engine is down. *(Empty pads are unaffected: `PadCell` returns at `:818` before the glow is created, so this is every filled pad, not all sixteen.)* | `KitScreen.kt:189-199`, `:818`, `:857-860`, `:153-173` |
...
| **J19** | **GROOVE's hand-editing door is mid-scroll, and its three entrances are scattered across three different rows.** `EDIT STEPS` (`:1989`) sits under `SHAPE THE GROOVE`, the second of the scroll region's four headings (`TRANSPORT`, `SHAPE THE GROOVE`, `RIDES PROG B`, `SEND IT SOMEWHERE`), below the swing stepper and feel row — while `RECORD` is correctly anchored below it. It is **not** the only route: `STEPS` in the anchored record row (`:1722`, `startSteps()`) and `EDIT THIS TAKE` in the anchored post-take row (`:1842`, `forkTakeToE()`) also enter PROG E, by two further code paths. Three doors to one editor, none of which names the other two, and only the buried one says what it does. | `GrooveScreen.kt:1989`, `:1722`, `:1842`, `:1939`, `:1992`, `:1871-1876`, `:2084-2088` |
...
| **J21** | **CHOP: returning to `BY HITS` from GRID, HUMMED or a ladder rung silently resets EAR, CUT and ON THE GRID — and the header stops printing them in the same frame.** `onByHits` (`:745-747`) falls through `hitsOf(...) ?: ByHits()`, and `hitsOf` returns null for exactly those three modes (`ChopReview.kt:686-691`), constructing fresh defaults. *(The GHOSTS round-trip is safe: `hitsOf(Ghosts)` returns the embedded `ByHits`, so those settings survive. The loss is real but narrower than a blanket “leave and return”.)* `modeLabel()` prints those three only when they differ from default (`ChopReview.kt:273-279`), so the evidence disappears simultaneously with the settings. | `ChopScreen.kt:745-747`, `:757-758`; `ChopReview.kt:110`, `:273-279`, `:689-691` |
...
| **J27** | **CHOP's CUT box goes dead a whole box at a time, without dimming.** Fifteen `SegmentButton`s wrap their own handler in `if (!busy)` — 5 on the zoom ladder (COUNT + four `Ladder.Rung`s), 3 ear, 3 cut, 4 grid-snap — and three more (BY HITS / GRID / GHOSTS) call through to `rechopTo`, which returns on `rechopBusy \|\| sendBusy \|\| humming` (`:432`). **HUM is the exception and stays live by design** — while humming it reads `STOP` and calls `stopHum` (`:811`), never `rechopTo`, so it is the one usable control in the box. How many of the rest are on screen is mode-dependent: the ladder row needs GRID/LADDER, the other three rows need a hits chop, so it runs from 3 dead controls to 18. No dim is possible — `SegmentButton` has no `enabled` — and `:1533` announces `selected` regardless. Prior #9, confirmed and wider. | `ChopScreen.kt:1299-1307`, `:1338-1368`, `:432`, `:1521-1541` |
...
| **J32** | **CHOP: `HUM` starts the take before it states the one rule that makes it work.** `startHum` does not start recording — it *requires* the mic to be armed already (`:557-559`), then starts a `TapeVoice` playing the source. That is what makes the ordering matter: in source order it is `v.start(0)` **then** `onToast(Copy.HUM_START)` (`:570-571`), so `"HUM ALONG. HEADPHONES ON, OR THE MIC HEARS THE TAPE TOO."` arrives when the tape is already audible and the ring is already capturing it — advice that can no longer be acted on without wrecking the take running. It is the only *proactive* statement of the headphone rule; `HUM_NO_MATCH` (`Personality.kt:795`) repeats it, but only after a take has already failed. The run is bounded by the whole source, not the selection, and the readout is `HUMMING…` with no elapsed time, so the user has committed to a performance of unknown length. | `ChopScreen.kt:566-572`, `:646`, `:1315`; `Personality.kt:789` |
...
| **J39** | **TAPE: the tape counter and the pencil rewind are invisible controls.** The position LCD is tappable but drawn as an LCD panel, not a button — a sighted user finds it only by tapping a readout for no reason. The left reel is a hold-only control with no *visible* text anywhere in it, sitting beside a visually identical right reel that has no semantics and no gesture at all. *(To TalkBack it is fine — `contentDescription = "REWIND PENCIL"` and an `onLongClick` labelled `SPIN BACK BY EAR`. This is a sighted-affordance failure specifically, and the gap between the two is the finding: the screen-reader user is told about a control the sighted user cannot see.)* | `TapeScreen.kt:1221-1238`, `:1401-1446`, `:1454-1459`, `:1492-1510` |
| **J40** | **TAPE: the `HIT 3/12` readout is secretly a third button that duplicates the one beside it.** Same `onClick` as `HIT ▶` (`stepHit(1)`, `:1039-1042`). It omits `DeckButton`'s optional `accessibilityLabel` override, so `tapeClick` falls back to the label — which here *is* the readout: TalkBack announces an actionable control named "HIT 3 OF 12". It has an accessible name; the name is a reading, not an action. | `TapeScreen.kt:1038-1043` |
...
*(Scope note, so the headline count stays auditable: the “47 findings”
figure counts the numbered table J1–J47 and nothing else. This section
and the two that follow it are synthesis over that table plus a few
concrete instances — the empty `CHOP FAILED` panel, the vanishing
`ONTO … · BANK B` control — that are deliberately unnumbered. Add them
to the total if you prefer; they are not double-counted in it.)*
...
`DeckButton` never pass `tapeClick`'s `enabled` flag, so up to eighteen
...
| 9, 10, 11, 12, 13, 24 | **Still true** | re-verified at current line numbers; see J27, J6, J43, J20, J42, and J41 |
| 15 (MERGE/SPLIT have no on-screen hint) | **Still true, and not carried into this review's table** | re-verified at `ChopScreen.kt:1121-1134`, but no J-number covers it: J28 is about MERGE/SPLIT being bright and dead *during a hum*, a different defect. Listing 15 here without a J-number is the honest bookkeeping — it is an open prior finding this pass did not re-state. |
