...
**Sixteen findings are closed, eight are decided, three are half-closed,
and two stand.** The two are P1.4 and P2.2, and both want five real users
for an afternoon rather than any code. The counts below are of the 22 numbered persona
...
| **P4.4** — provenance is good and half-hidden | Closed 2026-09-20, in two halves and with a defect found underneath. **The export half** (#291): a phone-made expansion now carries its J-Card and liner notes, as a CLI-made one always did. **The in-app half:** KIT prints where the kit came from under its grid — always there, cannot be dismissed, the same move S2 made for the pad-sheet gesture. Naming provenance in `PAD_SHEET_LEGEND` was looked at and rejected (that legend is a sample of what the sheet holds, not an inventory, and does not name layers or takes either), and a third mark on the grid would crowd J36's treated dog-ear and W12's mini-waveform for a fact a mark cannot state. **The defect:** the app had *three* readers of a pad's `source` map — the pad sheet's, the liner notes' and `Lineage`'s — with three key lists in two orders. A pad captured from another app showed as its bare filename on the pad sheet and as *"Built by hand, pad by pad."* in the notes that went on the card, and a sculpted or dissected kit came out of `snipsnap lineage` as *"made from scratch"*. `Provenance` (`:shell`, tested over every door) is the only reader now. |
| **P1.1**, X2 — "DUB IT" is the last word and it is opaque | Closed 2026-09-21. `FIRST_RUN_LOOP_NOTE`'s fourth verb is **WRITE IT**. DUB stays the app's word everywhere else — it is what EXPORT calls the write while it runs (`DUBBING…`, `DUB COMPLETE`) — but this is the one sentence written for somebody who has not used the app yet, and the word they arrive at is the one on the button: EXPORT's primary action reads `WRITE KIT`. J13's own rule (name what the screen names), applied to the last step instead of the third. |
| **P2.4** — the loop starts at "record" · **P3.2** — record-to-pad is not the advertised path | Closed 2026-09-21, and **both were aimed slightly wrong.** They ask for a second entry point to be advertised in the first-run note. Reading the screen: the second entry point is already the most prominent thing on it — `NEW KIT ▸ STARTERS`, the primary action directly under that panel — and the doors that skip the middle (`INSTANT KIT`, `CATCH A HIT`) are buttons on TAPE. Nothing is hidden, and the note's own KDoc argues against naming any of them, since a line pointing at STARTERS would be a third copy of a control already twice on screen. What *was* wrong is that the note ended `FOUR TABS, IN ORDER` — stricter than the app, since a starter kit begins at step three and either TAPE shortcut lands sounds on pads without CHOP. The clause now says so. One further real gap: P3.2 names `INSTANT KIT` and `CATCH A HIT` together, and **CATCH had a HELP line already while INSTANT KIT was in no list at all** — so half of it was closed before it was written and the other half was invisible. `HELP_MORE` names both now. |
...
| **P4.2** — they will notice what isn't there | The finding names **three** absences, and they have come apart. **The audio bounce is built** — GROOVE ▸ BOUNCE, as Headline 1 above records. **MIDI in/out and clock sync are accepted as a known gap:** `docs/MIDI_SYNC.md` opens with *"That decision unblocked this design pass. It does not unblock an implementation, and this document is the reason why,"* carries a section recording that three review rounds found 39 real errors in it — concentrated in its prescriptions, not its survey — and its last commit is titled *"Stop writing experiments for hardware I cannot touch."* Nothing needs marking as parked, because nothing there reads as a plan. **Sample-rate control was the limb nobody had ruled on. Ruled on 2026-09-20, and the ruling is that it has two halves that were being asked as one question.** *As a sound* it shipped long ago under a better name than a number: the TIME MACHINE is 26.04 kHz at 12 bits for SP1200 and 40 kHz for MPC60, on the pad sheet and in `snipsnap era` — which is what asking a sampler for a rate control usually means. *As a format* it is fixed at 44.1 kHz and will stay fixed: `WavWriter` refuses any other rate, `Preflight` FAILs a pad at one, and a picker offering a second setting the export would then refuse is a trap rather than a control. What was genuinely missing was neither — it was that **the app had never once stated the rate, and silently changed other people's.** Both are now fixed: SETUP carries a `THE RATE` row beside the three things finding 23 found it knew and never said, and `SnipStore.import` reports the rate a 48 k or 22.05 k share came in at instead of converting in silence. The number itself now has one owner (`WavWriter.MPC_SAMPLE_RATE`) and a `ConventionTest` law keeping it there — before that it was typed on seventeen code lines across six modules. |

| **P1.5** — all-caps sentences | **Accepted, 2026-09-21.** Not a bug: the caps are the app's voice, and `PERSONALITY.md` is built on them. One persona dislikes a house style five screens deep. Changing it is a rewrite of every string in `Personality.kt`, and nothing about the finding argues the style is *wrong* — only that it is not to one taste. |
| **P3.1** — thirteen destinations against roughly one | **Accepted as out of scope for this review, 2026-09-21.** Stale as written (it says twelve; SNAP made thirteen), and J12's grouping shipped — but grouping thirteen is not reducing thirteen, and the finding is right about that. It is a question about what the app *is*, not about the menu strip, so it is an initiative rather than a review follow-up. |
| **P3.3** — the app never says "resample" | **Accepted, 2026-09-21.** Confirmed by search: the word appears only as `Resampler` in code and in one refusal string, never as a control. That is deliberate vocabulary and S1's own verdict supports keeping it. Recorded so a future reader does not mistake it for an oversight. |
| **P5.3** — punch-in FX vs macros-and-print | **Accepted, 2026-09-21.** Architectural. SURFACE's output is a print, not a live effect on a master bus, and changing that is a different product rather than a fix to this one. |
| **S1** — keep the cassette metaphor | **Accepted, 2026-09-21.** A recommendation the app already follows, and independently settled the same way by J39: `PERSONALITY.md` catalogs the reels as hidden eggs under its own law 4. |

## Standing

| Finding | Checked |
|---|---|
| **P1.4**, X6 — the vocabulary wall, and HELP | Counted: `HELP_LOOP` is 4 items, `HELP_MORE` is 21 (20 when this was written; P3.2's fix added the INSTANT KIT line), HELP is 13th of thirteen. The wall is as described, and wider than when it was measured. |
