...
| # | Finding | Evidence |
|---|---|---|
| **6** | **Tilt in MORPH mode is invisible.** The commit touched `SurfaceEngine.cpp` and `SurfaceStore.kt` only. The readout prints a `TILT` number in every mode, and did before this commit, so it says the angle is *measured*, not that it now shapes MORPH; no toast, legend, HELP line or mode copy says it does. There is no tilt on/off control anywhere in the app, so a user holding the phone at an angle in MORPH hears a resonance offset on every corner with no explanation and no way out. The screen's own KDoc still says tilt is XYZ-only. | `git show --stat 074ac99`; `SurfaceScreen.kt:595` (same line at `074ac99~1`), `:79-82`; `Personality.kt:406` (the one HELP line) |
...
Then, cheaply: ~~a toast on GROOVE's disabled BOUNCE (#8)~~ (done 2026-09-13), a `selected` state on SURFACE's modes and a `TILT` word in MORPH's copy (#6, #21), and the four stale documents (#23).
