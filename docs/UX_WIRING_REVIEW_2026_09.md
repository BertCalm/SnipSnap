...
| **3** | ~~**The CATCH grid on TAPE is wider than a phone held upright.**~~ **Fixed 2026-09-13.** The finding held, and more firmly than written: the app is portrait-locked at the manifest everywhere but PLAY's fullscreen route, so `BankRow`'s 888 dp floor was never met on TAPE and the grid *always* fell to the scroll branch. CATCH now draws one bank as the 4×4 window KIT, PLAY and GROOVE already draw (`PadGrid.windowRows`), under a `BANK A · 3 FREE` / `BANK B · 16 FREE` switch that counts the pads a catch can still land on and opens on the first bank with one. With no free pad on either bank the button refuses in words before the loop starts. `docs/CATCH.md` says what the grid is now. Layout read off the rules, not a device. | was `TapeScreen.kt:929` (`BankRow`); now `PlayBank(windowRows(catchBank))` under the bank switch, `Copy.catchBank`, `Copy.CATCH_NO_ROOM` |
