...
**The wiring is sound.** All seventeen features in the table at the
end have every callback connected, with no no-op defaults at their call
site. Where they fall short it is never the wiring: one refusal names a
control that is not on any screen (#4), one set of toggles speaks a name
but no state (#20), and one feature has no control at all (#6). The a11y
pass held: the new work got the same treatment the old work did, with
three exceptions listed under S2/S3 (#20, #21, #29).
...
| # | Finding | Evidence |
|---|---|---|
| **1** | ~~**HEAR plays MUTATE's preview an octave low at half speed.**~~ **Fixed 2026-09-13.** The finding held: `Mutate.render` up-mixes both parents to two channels and returns interleaved stereo, and `onHear` handed that straight to `audition()`, a `CHANNEL_OUT_MONO` `TapeVoice` over a bare `FloatArray`, so every frame played twice. The fold went into `audition()` itself rather than into `onHear`: the voice is mono, so the door to the voice is where the fold belongs, and the next play path added to the sheet cannot forget it. A no-op for the pad's own file and the BEFORE take, which were already folded where they are read. Not phone-verified — `:app` compiles only in CI — but the arithmetic is the whole bug: two channels into one. | was `PadSheetScreen.kt:1288-1290` → `:321-331`; now `Cleanup.toMono` at the top of `audition()` |
| **2** | ~~**GROOVE plays a non-4/4 ORBIT clip as 4/4, and shows no meter.**~~ **Fixed 2026-09-13.** The finding held, and went one layer deeper than the screen: `LiveRecord.Take` wrapped every hit at 4/4 too, so an overdub on a 3/4 base put hits in a fourth beat the clip did not have and `toClip` refused the whole take. Now the clip is asked, not assumed: `GrooveEdit.stepsInClip` is the loop length the clock wraps at, `stepsPerBar` is the accent, the bar counters and the step editor's column count (twelve cells for 3/4, twenty for 5/4), and a take is played `against` the base's own bar and lands with its meter. The header reads `2 BARS · 3/4` and the landing toast `OVER 2 BARS OF 3/4` — only when the meter is not 4/4, the way `groove.json` omits the key for 4/4, so the label appears exactly when a player would otherwise assume wrong. Model-tested in `:kit`; the screen arithmetic is the same helpers, not phone-verified. | was `GrooveScreen.kt:892`, `:955`, `:1617`, `:1679`, `LiveRecord.kt:71-78`; now `GrooveEdit.stepsInClip` / `stepsPerBar` / `meterLabel`, `LiveRecord.Take.against` |
| **3** | ~~**The CATCH grid on TAPE is wider than a phone held upright.**~~ **Fixed 2026-09-13.** The finding held, and more firmly than written: the app is portrait-locked at the manifest everywhere but PLAY's fullscreen route, so `BankRow`'s 888 dp floor was never met on TAPE and the grid *always* fell to the scroll branch. CATCH now draws one bank as the 4×4 window KIT, PLAY and GROOVE already draw (`PadGrid.windowRows`), under a `BANK A · 3 FREE` / `BANK B · 16 FREE` switch that counts the pads a catch can land on (empty when CATCH began; a caught pad stays catchable) and opens on the first bank with one. With no free pad on either bank the button refuses in words before the loop starts. `docs/CATCH.md` says what the grid is now. Layout read off the rules, not a device. | was `TapeScreen.kt:929` (`BankRow`); now `PlayBank(windowRows(catchBank))` under the bank switch, `Copy.catchBank`, `Copy.CATCH_NO_ROOM` |
| **4** | ~~**HUM's refusal names a control that does not exist.**~~ **Fixed 2026-09-13.** The finding held. All three HUM refusals now name the button as it reads and the tab it is on: `THE MIC ISN'T LISTENING. LISTEN · MIC ON KITS, THEN HUM.`, the INSIDE one with the step it needs first (`STOP ON KITS, THEN LISTEN · MIC, THEN HUM.` — while a session runs KITS shows STOP in the button's place, and arming again only re-enters the running session), and the heard-nothing one `… LISTEN · MIC ON KITS, THEN HUM AGAIN.` (its old "ARM IT" had the same problem). `docs/CHOP_CONTROLS.md` §10 now quotes the words the screen draws. | was `Personality.kt:752-760`; now `HUM_NOT_LISTENING`, `HUM_INSIDE`, `HUM_NOTHING` |
| **5** | ~~**GRAIN ▸ is very likely drawn at zero height.**~~ **Fixed 2026-09-13.** The button carried `Modifier.weight(1f)` while its sibling MAKE INSTRUMENT was `fillMaxWidth()`, a leftover from a `Row`. It sits in `GroupBox`'s content `Column`, inside the sheet's `verticalScroll` column, where the incoming max height is unbounded; Compose gives a weighted child of an unbounded column zero main-axis space, and `heightIn(min = 48)` cannot exceed an incoming max of 0. This is the **only door into GRAIN FIELD** (`grainFieldSlot` is set nowhere else), and `PAD_SHEET_LEGEND` promises GRAIN. The weight is `fillMaxWidth()` now, with a comment on why it can never be a weight here. Still read off the layout rules, not a device: the rule is certain, and a screenshot of the MAKE box would confirm the door is drawn. | was `PadSheetScreen.kt:2421-2427`; `GroupBox.kt:61`; `App.kt` (sole setter) |

### S2 — friction, hidden features, silent refusals

| # | Finding | Evidence |
|---|---|---|
| **6** | ~~**Tilt in MORPH mode is invisible.**~~ **Fixed 2026-09-13.** The screen's own KDoc no longer says tilt is XYZ-only — it now names the half-weighted nudge XY and MORPH both get, against `SurfaceEngine::applyControl`'s actual per-mode switch. HELP gained one line: "TILT THE PHONE ON SURFACE FOR RESONANCE, EVEN IN MORPH." There is still no on/off control (this review's finding was about it being unexplained, not about adding one), but a user now has words for what they are hearing. | was `SurfaceScreen.kt:79-82`; `Personality.kt:406` (the one HELP line); now the KDoc plus a new `HELP_MORE` line |
...
| **8** | ~~**GROOVE's BOUNCE greys out with no reason.**~~ **Fixed 2026-09-13.** `playing` came out of the `enabled` gate — the button stays tappable on a stopped loop, and the tap now toasts `Copy.GROOVE_BOUNCE_NEEDS_PLAY` ("BOUNCE PRINTS THE LOOP AS IT PLAYS. PRESS ► PLAY FIRST.") instead of arming, the same toast-beats-gating move RECORD already made for its own `!bankReady` case. The other conditions (`currentClip`, `bankReady`, `recording`, `countingIn`, and the in-flight bounce states) stay in the gate — they are transient or resolve themselves, the way RECORD's `busy`/`recording`/`countingIn` guard does with a silent return. | was `GrooveScreen.kt:1853-1855` vs `:1340-1345`; now the same enabled gate minus `playing`, plus `Copy.GROOVE_BOUNCE_NEEDS_PLAY` |
...
| **14** | ~~**Two toasts point into a closed box.**~~ **Fixed 2026-09-13, for the one that needed it.** The no-hits toast now reads `NO HITS IN THIS SOURCE. OPEN CUT AND TRY GRID, OR TAPE SOMETHING ELSE.` The AUTO-none toast is left as it was: AUTO is a control inside the CUT box, so the box is open when that toast fires and GRID is on screen beside it. | was `Personality.kt:122`; now `CHOP_NO_HITS`; `CHOP_AUTO_NONE` unchanged, on purpose |
...
| **30** | **Group-box legends are repeated as card titles inside the box** ("TREATMENT" then "TREATMENT"; likewise MUTATE, OUTSIDE, SHAPE), where `UI_DESIGN.md` says boxes *replace* cards. | `PadSheetScreen.kt:2917`, `:3074`, `:3142`, `:3410`; `UI_DESIGN.md:168` |
| **31** | **OUTSIDE's cancel does not stop the mic.** The KDoc keeps the trip on the composable's scope so "a tab switch stops the mic", but `OutsideSession.run` is a blocking `read` loop with no cancellation check; the mic runs to `listenFrames`, then the return is discarded with no toast. The trip is a few seconds, so the cost is small; the stated reason is false. | `PadSheetScreen.kt:1507-1516`, `:1620`; `OutsideSession.kt:99-122` |
| **32** | **Stale KDoc and doc lines.** `CutBench` doc lists two modes (there are four plus the ladder); `SegmentButton` doc still says "CLASSIC/MELODIC, Batch 3"; `MutateCard` says four moves and two partner kinds (six and five); `PadSheet.kt:11` says five segments (six); `UAT_2026_09.md:74` and `:174` say 20 segments (`ALL_SEGMENTS` is 21 since DUST); `Hum.Reading.pattern` exists "for a later READ AS GROOVE" and is read only by a test; the SURFACE explainer boards predate the tilt commit and show a `(FLAT)` suffix the readout never prints. | `ChopScreen.kt:1226-1229`, `:1490-1503`; `PadSheetScreen.kt:3095-3097`; `Hum.kt:70-73`; `design/surface-explainer/SetCorner.dc.html:34` |
...
| GROOVE BOUNCE | ✓ | TRANSPORT group under PLAY | `BOUNCE` / `WAITING…` / `BOUNCING…` | toast on landing, and now on a stopped-loop tap (#8) | Destination still unnamed (#25) |
...
1. ~~`Cleanup.toMono` before `audition` in `onHear` (#1). One line.~~ Done 2026-09-13, inside `audition` itself.
2. ~~Read `pulsesPerBar` in GROOVE and use it for loop length, accents and the bar counter — or refuse a non-4/4 clip in words until it does (#2).~~ Done 2026-09-13, through `GrooveEdit`'s helpers; the recorder learned the bar too.
3. ~~Give CATCH a portrait layout: the 4×4 window grid PLAY already uses in portrait, or a bank toggle (#3).~~ Done 2026-09-13: both, the window under a bank switch.
4. ~~Reword `HUM_NOT_LISTENING` to name `LISTEN · MIC` on KITS (#4), and `CHOP_NO_HITS` to say "OPEN CUT, TRY GRID" (#14).~~ Done 2026-09-13, and the two other HUM refusals with it.
5. ~~Confirm #5 with one screenshot; if GRAIN ▸ is missing, drop the `weight(1f)` for `fillMaxWidth()`.~~ Done 2026-09-13 on the layout rule alone; the screenshot is now a confirmation, not a gate.
