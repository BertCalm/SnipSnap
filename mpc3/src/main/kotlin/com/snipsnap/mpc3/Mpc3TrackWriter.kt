...
class Mpc3TrackWriter(
    ...
    /** The `.xtd` container bytes. */
    fun write(program: DrumProgram, trackColour: Int = DEFAULT_TRACK_COLOUR): ByteArray =
        Acvs.write(
            AcvsHeader(firmware, Mpc3Project.TRACK_OBJECT_TYPE, AcvsHeader.ENCODING_JSON, platform),
            payloadText(program, trackColour),
        )
     ...
     */
    fun writeTo(directory: File, program: DrumProgram, trackColour: Int = DEFAULT_TRACK_COLOUR): File {
        require(directory.isDirectory) { "not a directory: $directory" }
        val file = File(directory, "${program.name}.xtd")
        file.writeBytes(write(program, trackColour))
        return file
    }

    /** The uncompressed JSON payload, for tests and diffing. */
    fun payloadText(program: DrumProgram, trackColour: Int = DEFAULT_TRACK_COLOUR): String =
        render(payload(program, trackColour))
    ...
    private fun payload(program: DrumProgram, trackColour: Int): J = obj(
        "data" to obj(
            "version" to i(5),
            "name" to s(program.name),
            "volume" to d(1.0),
            ...
            "samples" to samplesPool(program),
            "program" to programObj(program),
            "lengthFollowsSequenceLength" to b(true),
            "midiEventsFilter" to midiEventsFilter(),
            "arrangementClipMap" to arrangementClips(program.name),
            "sharedClipMap" to J.A(emptyList()),
            "recordArm" to b(true),
     ...
     */
    private fun samplesPool(program: DrumProgram): J {
    ...
    private fun programObj(program: DrumProgram): J = obj(
        "version" to i(2),
        "name" to s(program.name),
        "type" to i(0),
        "programPads" to programPads(program),
        "customQLinks" to J.A(List(16) { emptyQLink() }),
        "transpose" to i(0),
        "midiKillGroup" to i(-1),
        "chainID" to i(0),
        "midiEventsFilter" to midiEventsFilter(),
        "mixable" to mixable(level = program.level.toDouble(), pan = program.pan.toDouble(), destination = 2),
        "customisable" to obj("mapping" to J.A(emptyList())),
        "renderable" to obj("sendToCueBus" to b(false)),
        "xfaderRoute" to i(0),
        "padNoteMap" to padNoteMap(),
        "drum" to drumObj(program),
        "fxRackQLinks" to J.A(emptyList()),
        "customMacroSceneData" to macroScenes(),
        "fxRackMacroSceneData" to macroScenes(),
    )
    ...
    /** MPC 3's default map is chromatic from 36 with wraparound — pad A01 = note 36. */
    private fun padNoteMap(): J = obj(
        "noteForPad" to obj(
            *(0 until 128).map { n -> "value$n" to i(((36 + n) % 128).toLong()) }.toTypedArray(),
        ),
    )

    private fun drumObj(program: DrumProgram): J = obj(
        "version" to i(8),
        "pitch" to d(0.0),
        "coarseTune" to i(0),
        "fineTune" to i(0),
        "monophonic" to b(false),
        ...
        "instruments" to J.A((0 until 128).map { instrument(program.pads.getOrNull(it)) }),
        "driftSpeed" to d(0.0),
    ...
    private fun instrument(pad: Pad?): J {
        ...
        return obj(
            "version" to i(15),
            "coarseTune" to i((pad?.tuneCoarse ?: 0).toLong()),
            "fineTune" to i((pad?.tuneFine ?: 0).toLong()),
            "monophonic" to b(false),
            "polyphony" to i(3),
            "lowNote" to i(0),
            "highNote" to i(127),
            "ignoreBaseNote" to b(false),
            "zonePlayTime" to i(1),
            "whichMuteGroup" to i((pad?.muteGroup ?: 0).toLong()),
            "muteTargets" to obj(*(0 until 4).map { "value$it" to i(0) }.toTypedArray()),
            "simultPlayTargets" to obj(*(0 until 4).map { "value$it" to i(0) }.toTypedArray()),
            "synthSection" to synthSection(filled),
            // 0 = One Shot (the whole sample fires), 2 = Note On (sustains
            // while held) — a per-pad musical choice in real kits.
            "triggerMode" to i(if (pad?.oneShot != false) 0 else 2),
            "tempo" to d(120.0),
            "bpmLock" to b(true),
            "warpEnable" to b(false),
            "stretchPercentage" to i(100),
            "layersv" to J.A(
                (0 until 8).map { slot -> layer(zones.getOrNull(slot), filledInstrument = filled) },
            ),
            "editAllLayers" to b(false),
            "articulationType" to i(0),
            "articulationSpeed" to d(0.5),
            "userSelectableWarpPoolIndex" to i(0),
            "mixable" to mixable(
                level = pad?.level?.toDouble() ?: MPC_LEVEL_EMPTY,
                pan = pad?.pan?.toDouble() ?: 0.5,
                destination = 0,
            ),
            "padEffects" to padEffects(),
        )
    }

    private fun layer(zone: VelocityLayer?, filledInstrument: Boolean): J = obj(
        "active" to b(true),
    ...
    private fun synthSection(filled: Boolean): J = obj(
        "version" to i(15),
        "filterData" to obj(
            "value0" to filterSlot(filterType = 2),
            "value1" to filterSlot(filterType = 0),
        ),
        "filterSerialRouting" to b(false),
        "filterBlend" to d(0.5),
        "lfoData" to obj(
            "value0" to lfoSlot(reset = !filled),
            "value1" to lfoSlot(reset = true),
        ),
    ...
    private fun filterSlot(filterType: Int): J = obj(
        "version" to i(15),
    ...
    private fun lfoSlot(reset: Boolean): J = obj(
        "version" to i(15),
        "lfoPitch" to d(0.0),
    ...
    private fun envelope(decay: Double, decayFromEnd: Boolean): J = obj(
        "version" to i(2),
        "Attack" to v0(d(0.0)),
