 ...
 * others wait on the same future. A render that fails, or is cancelled
 * because the grid left its tempo, is forgotten, so the next bake tries
 * again; it answers null, which the baker plays as silence.
 *
 * Renders are kept mono. The baker widens each slice to stereo as it cuts
 * it, so a stereo copy here would double the largest thing a drone holds.
 */
class DroneSource(
    ...
    override fun drone(
        ...
        return try {
            ...
            // A future completed with a CancellationException rethrows it as
            // itself from get(), not wrapped: a stopped render lands here.
            null
